...
def predict_game(avg_woba, avg_score, opp_era, opp_win_pct, opp_lineup_woba=None):
    w = WEIGHTS
    ...
    predicted_runs  = avg_woba * w["woba_run_scale"] * score_factor * adj_era_factor
    ...
    win_prob        = 1 / (1 + math.exp(-run_diff * 0.55))
    return {
...
# ── Load roster: season avg wOBA per player ───────────────────────────────────
print("Loading player_gamelogs for roster wOBA…")
cur.execute("""
    SELECT player_name, team,
           AVG(woba) FILTER (WHERE woba > 0) AS avg_woba,
...
# name → {team, avg_woba, games_played}
roster_map = {}
for name, team, avg_woba, gp in roster_rows:
    roster_map[name] = {"team": team, "avg_woba": float(avg_woba) if avg_woba else 0.310, "games_played": gp}
...
for name, info in roster_map.items():
    ...
    if t not in team_roster:
        team_roster[t] = []
    team_roster[t].append({"name": name, "avg_woba": info["avg_woba"]})
...
print(f"Loading game_results {start_date} → {today - timedelta(days=1)}…")
cur.execute("""
...
for row in games_raw:
    ...
    for is_home in [True, False]:
        ...
        l10  = opp_info.get("l10_win_pct") or opp_info.get("win_pct") or 0.500
        seas = opp_info.get("win_pct") or 0.500
        opp_win_pct = (l10 * 0.5 + seas * 0.5) if opp_info.get("l10_win_pct") else seas

        # Actual lineup wOBA from roster map
        actual_wobas  = [roster_map.get(p["name"], {}).get("avg_woba", 0.310) for p in my_lineup if p.get("name")]
        avg_actual_woba = spot_weighted_woba(actual_wobas)
        opp_woba_f = float(opp_woba) if opp_woba else None
        pred_actual = predict_game(avg_actual_woba, 50, opp_era, opp_win_pct, opp_woba_f)
        ...
        pred_sug = predict_game(avg_sug_woba, 50, opp_era, opp_win_pct, opp_woba_f)
...
for team, rows in game_rows_by_team.items():
    ...
    try:
        ...
        print(f"  ✓ {team}: {len(rows)}g  W/L {acc}  RunMAE {primary['avg_run_mae'] if primary else '—'}")
        saved += 1
